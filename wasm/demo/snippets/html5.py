...
from browser import jsstr, JsValue
jseval = None  # this has to be implemented!
...
def domCreateElement(tag, ns=None):
	...
	if uri:
		return document.get_prop("createElementNS").call((jsstr(uri), jsstr(tag)), this=document)

	return document.get_prop("createElement").call(jsstr(tag), this=document)


def domCreateTextNode(txt=""):
	return document.get_prop("createTextNode").call(jsstr(txt), this=document)


def domGetElementById(idTag):
	return document.get_prop("getElementById").call(jsstr(idTag), this=document)


def domGetElementsByTagName(tagName):
	items = document.get_prop("getElementsByTagName").call(jsstr(tagName), this=document)
...
class Widget(object):
	...
	def addEventListener(self, event, callback):
		...
		"""
		self.element.get_prop("addEventListener").call(jsstr(event), callback)
	...
	def removeEventListener(self, event, callback):
		...
		"""
		self.element.get_prop("removeEventListener").call(jsstr(event), callback)
	...
	def appendChild(self, *args, **kwargs):
		...
		for child in toAppend:
			...
			print(f"trying to append {child.element} to {self.element}")
			self.element.get_prop("appendChild").call(child.element, this=self.element)
			child._parent = self
	...
	def removeChild(self, child):
		...
		self.element.get_prop("removeChild").call(child.element, this=self.element)
	...
	def focus(self):
		self.element.get_prop("focus").call(this=self.element)

	def blur(self):
		self.element.get_prop("blur").call(this=self.element)
		...
		def __init__(self):
			super().__init__(
				...
				<label>
					...
					<input [name]="nameInput" @change="onNameChanged">
				</label>
				<h3>Your Name is: <span [name]="nameOutput"></span></h3>
				"""
			)
